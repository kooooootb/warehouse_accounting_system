...
GetUserSize::GetUserSize(std::shared_ptr<srv::ITracer> tracer, std::shared_ptr<srv::IServiceLocator> locator, const TaskInfo& taskInfo)
    : BaseTask(std::move(tracer), std::move(locator), std::move(taskInfo))
{
}

ufa::Result GetUserSize::ExecuteInternal(std::string& result)
{
    ...
    const auto createResult = ActualGetUserSize(*accessor, count);
...
void GetUserSize::ParseInternal(json&& json)
{
    TRACE_INF << TRACE_HEADER << "Parsing " << GetIdentificator();
}

ufa::Result GetUserSize::ActualGetUserSize(srv::IAccessor& accessor, int32_t& count)
{
    ...
    auto query = QueryFactory::CreateRaw(GetTracer(), "SELECT COUNT(*) FROM public.\"User\";", {});

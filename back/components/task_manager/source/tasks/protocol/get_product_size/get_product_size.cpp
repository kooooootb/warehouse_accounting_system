...
GetProductSize::GetProductSize(std::shared_ptr<srv::ITracer> tracer,
...
ufa::Result GetProductSize::ExecuteInternal(std::string& result)
{
    ...
    const auto createResult = ActualGetProductSize(*accessor, count);
...
void GetProductSize::ParseInternal(json&& json)
{
    TRACE_INF << TRACE_HEADER << "Parsing " << GetIdentificator();
}

ufa::Result GetProductSize::ActualGetProductSize(srv::IAccessor& accessor, int32_t& count)
{
    ...
    auto query = QueryFactory::CreateRaw(GetTracer(), "SELECT COUNT(*) FROM public.\"Product\";", {});
